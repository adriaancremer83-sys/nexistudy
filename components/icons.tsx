// Heroicons (MIT) outline paths, 24x24 viewBox. One consistent set so the UI
// never falls back to emoji glyphs that render differently per platform.

interface IconProps {
  className?: string;
}

function Base({ className = "w-6 h-6", children }: IconProps & { children: React.ReactNode }) {
  return (
    <svg
      className={className}
      fill="none"
...
export function IconCpuChip({ className }: IconProps) {
  return (
    <Base className={className}>
